...
inherit autotools eutils gnustep-base flag-o-matic mercurial
...
EHG_REPO_URI="https://hg.windowmaker.info/wmaker"
...
SRC_URI="http://www.windowmaker.info/pub/source/release/WindowMaker-extra-0.1.tar.gz
	http://www.gentoo.org/~grobian/distfiles/${PN}-${MY_PV}-patchset-${PATCHVER}.tar.bz2"
...
src_unpack() {
	...
	unpack ${A}
	cd "${S}"
	local psd="${WORKDIR}"/${PN}-${MY_PV}-patchset-${PATCHVER}

#	epatch "${psd}"/WindowMaker-0.92.0-cvs20060123.patch
	epatch "${psd}"/WindowMaker-0.92.0-cvs-gcc41.patch
	epatch "${FILESDIR}"/${MY_PV/0.92/0.91}/wlist-0.9x.patch
	epatch "${FILESDIR}"/${MY_PV}/${PN}-${MY_PV}-gif-before-ungif.patch
	epatch "${FILESDIR}"/${MY_PV}/${PN}-${MY_PV}-math_h.patch
